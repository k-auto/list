...
def encrypt_file(input_file: str, output_file: str, password, layers: int = 1):
    import os, shutil, tempfile, struct, zlib
    ...
    for layer in range(layers):
        ...
        try:
            ...
            metadata_bytes = b"".join(metadata)
            metadata_len = struct.pack(">Q", len(metadata_bytes))
            final_file = tempfile.NamedTemporaryFile(delete=False)
            final_file_name = final_file.name
            final_file.close()
            try:
                with open(final_file_name, 'wb') as f_out, open(temp_out_name, 'rb') as enc_f:
                    f_out.write(metadata_len)
                    f_out.write(metadata_bytes)
                    while True:
                        buf = enc_f.read(CHUNK_SIZE)
                        if not buf:
                            break
                        f_out.write(buf)
            finally:
        ...
        for layer in range(layers):
            ...
            try:
                ...
                with open(current_input, 'rb') as f, open(temp_out_name, 'wb') as out_f:
                    ...
                    for i in range(0, metadata_len, SALT_SIZE + NONCE_SIZE + 8):
                        salt = metadata_bytes[i:i+SALT_SIZE]
                        nonce = metadata_bytes[i+SALT_SIZE:i+SALT_SIZE+NONCE_SIZE]
                        length = struct.unpack(">Q", metadata_bytes[i+SALT_SIZE+NONCE_SIZE:i+SALT_SIZE+NONCE_SIZE+8])[0]
                        if total_processed % KEY_ROTATION_SIZE == 0:
                            key = derive_key(password, salt)
                            aesgcm = AESGCM(key)
...
def archive_folder(target_folder: str, output_archive: str, ignore=None):
    ...
    with zipfile.ZipFile(output_archive, "w", zipfile.ZIP_DEFLATED) as z:
        for p in target_path.rglob("*"):
            ...
            if any(rel_path.startswith(ig) for ig in ignore_set):
                continue
            z.write(p, arcname=str(Path(target_path.name) / rel_path))
...
def createkp(folder, key=None, location=None):
    folder_path = Path(folder).resolve()
...
Example `pkgdeps.json`: `{"mypackage1":{"author":"github_user","key":null,"location":null},"mypackage2":{"filepath":"local_folder/mypackage2.kp","key":"mysecretkey","location":"packages/mypackage2"}}`
...
def openkp(package, key=None, location=None):
    ...
    with open(temp_decrypted, "rb") as f:
        decrypt_file(temp_decrypted, temp_decrypted, key)
    ...
    if pkgdeps_path.exists():
        ...
        for dep, meta in deps.items():
            ...
            if dep_location:
                dep_location = str((package_root / dep_location).resolve())
            if "author" in meta:
                getpkg(meta["author"], dep, meta.get("key"), dep_location)
            elif "filepath" in meta:
                openkp(str(Path(meta["filepath"]).resolve()), meta.get("key"), dep_location)
    build_path = Path(location) / "kpcore" / "build.py"
    subprocess.run([os.sys.executable, str(build_path)], cwd=str(location), check=True)
...
def main():
    ...
    if args.command == "getpkg":
        ...
        createkp(args.folder, args.key, args.location)
